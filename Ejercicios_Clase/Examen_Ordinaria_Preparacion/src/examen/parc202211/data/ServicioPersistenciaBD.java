...
public class ServicioPersistenciaBD implements ServicioPersistenciaDeustoBeReal {
	...
	public Set<Categoria> buscarCategoriasDeUsuario( String nickUsuario ) {
		...
		String sql = "select categoria from foto where usuario = '" + nickUsuario + "'";
		try {
			// Búsqueda de todas las fotos del usuario
			log( Level.INFO, "Lanzada consulta a base de datos: " + sql, null );
			ResultSet rs = statement.executeQuery( sql );
			while (rs.next()) {
				int codCat = rs.getInt("categoria");
				// Búsqueda de categoría de cada foto
				sql = "select * from categoria where codigo = " + codCat;
				Statement stat2 = connection.createStatement();
				log( Level.INFO, "Lanzada consulta a base de datos: " + sql, null );
				ResultSet rs2 = stat2.executeQuery( sql );
				Categoria categoria = new Categoria( rs2.getInt("codigo"), rs2.getString("nombre"), rs2.getInt("numFotos") );
				ret.add( categoria );
				rs2.close();
				stat2.close();
			}
