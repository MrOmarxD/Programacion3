...
public class OneWorldService extends AirAllianceService {
	...
	private static final String BOARDINGPASSES_FILE = "resources/data/boardingPasses.csv";
	...
	public void storeBoardingPass(BoardingPass boardingPass) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(BOARDINGPASSES_FILE, true))) {
			String line = String.format("%s#%s#%s#%s", 
									boardingPass.getDate(),
	...
	private Map<String, List<Reservation>> loadReservationsCSV() {
		Map<String, List<BoardingPass>> boardingPasses = loadBoardingPassesCSV();
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(RESERVATIONS_FILE))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				passes = boardingPasses.get(reservation.getLocator());
				
				if (passes != null) {
					reservation.setBoardingPasses(passes);
				}
	...
	/**
	 * Devuelve un mapa que indexa la lista de tarjetas de embarque por localizador de reserva.
	 * @return Map<String, List<BoardingPass>> con las tarjetas de embarque.
	 */
	private Map<String, List<BoardingPass>> loadBoardingPassesCSV() {
		Map<String, List<BoardingPass>> boardingPasses = new HashMap<>();
		
		try (BufferedReader reader = new BufferedReader(new FileReader(BOARDINGPASSES_FILE))) {
			String line = reader.readLine();
			BoardingPass boardingPass;

			while ((line = reader.readLine()) != null) {
				boardingPass = BoardingPass.parseCSV(line);
				boardingPasses.putIfAbsent(boardingPass.getLocator(), new ArrayList<>());
				boardingPasses.get(boardingPass.getLocator()).add(boardingPass);
			}
		} catch (Exception ex) {
			logger.warning(String.format("%s - Error cargando reservas: %s", getAirAlliance(), ex.getMessage()));
		}
