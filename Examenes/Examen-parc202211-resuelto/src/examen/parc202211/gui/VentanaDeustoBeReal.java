...
public class VentanaDeustoBeReal extends JFrame {
	...
	// T3 - posibles atributos necesarios
	...
	public VentanaDeustoBeReal( ServicioPersistenciaDeustoBeReal servicio ) {
		...
		tFotos.setDefaultRenderer( Object.class, (table, value, isSelected, hasFocus, row, column) -> {
		...
		tFotos.addMouseMotionListener( new MouseMotionAdapter() {
			@Override
			public void mouseMoved(MouseEvent e) {
				filaMouseOver = tFotos.rowAtPoint(e.getPoint());
				colMouseOver = tFotos.columnAtPoint(e.getPoint());
		...
		tFotos.addKeyListener( kl );
		lUsuarios.addKeyListener( kl );
		lCategorias.addKeyListener( kl );
		bCargar.addKeyListener( kl );
	}
	...
	public static List<String> cargarDatos( ServicioPersistenciaDeustoBeReal servicioPersistencia, File file ) {
		// T1 - carga de datos de fotos de fichero de texto
		List<String> errores = new ArrayList<>();
		try {
			int siguienteCodigoFoto = servicioPersistencia.getMayorCodigoFoto() + 1;
			...
			while (scanner.hasNextLine()) {
				...
				if (partes.length!=4) {
					String error = "Línea " + numLinea + ": formato incorrecto";
					errores.add( error );
					continue;
				}
				Usuario usuario = servicioPersistencia.buscarUsuarioPorNick( partes[0] );
				if (usuario == null) {
					String error = "Línea " + numLinea + ": no existe usuario";
					errores.add( error );
					continue;
				}
				try {
					int codigo = Integer.parseInt( partes[1] );
					Categoria categoria = servicioPersistencia.buscarCategoriaPorCodigo( codigo );
					if (categoria==null) {
						throw new NullPointerException();  // Mismo error que código de categoría no numérico - se gestiona en este próximo catch
					}
					try {
						long fecha = Long.parseLong( partes[2] );
						String rutaFoto = partes[3];
						File fileFoto = new File( rutaFoto );
						if (!fileFoto.exists()) {
							String error = "Línea " + numLinea + ": no existe foto";
							errores.add( error );
							continue;
						}
						Foto foto = new Foto( siguienteCodigoFoto, usuario, categoria, fecha, rutaFoto );
						servicioPersistencia.insertarFoto( foto );
						siguienteCodigoFoto++;
					} catch (NumberFormatException e) {
						String error = "Línea " + numLinea + ": fecha incorrecta";
						errores.add( error );
					}
				} catch (NullPointerException | NumberFormatException e) {
					String error = "Línea " + numLinea + ": no existe categoría";
					errores.add( error );
				}
			}
			scanner.close();
