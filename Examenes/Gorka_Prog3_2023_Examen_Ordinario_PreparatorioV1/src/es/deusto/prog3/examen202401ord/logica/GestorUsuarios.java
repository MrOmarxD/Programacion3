...
public class GestorUsuarios {
	...
	/*ANDONI: Hemos añadido que si se inicia sesión desde admin admin, se abre la ventanaAdmin con la tabla*/
	public void procesoPostLogin() {
		if (ventLogin!=null) {
	        if (getUsuarioLogueado()==null) {
	        	...
	        	if(getUsuarioLogueado().getNick().equals("admin") && getUsuarioLogueado().getPassword().equals("admin"))
	...
	public void nuevoChat() {
		...
		String texto = ventChat.getTaChat().getText(); //Recogemos la pregunta hecha por el usuario/*ANDONI*/
		...
		String respuesta = chatNoGPT.devuelveRespuesta( texto ); //Se genera la respuesta
	...
	public GestorPersistencia getBd() {
		return bd;
	}
	
	/*ANDONI
	 * 
	 * Método que nos da acceso a la BBDD*/
